...
#section[
	...
	*Relationship Types:*
	+ *REGION 1:N STORE:* One-to-Many (ภูมิภาคหนึ่งสามารถมีร้านค้าได้หลายร้าน)
	+ *STORE 1:N EMPLOYEE:* One-to-Many (ร้านค้าหนึ่งสามารถจ้างพนักงานได้หลายคน)
	+ *JOB 1:N EMPLOYEE:* One-to-Many (ตำแหน่งงานหนึ่งสามารถถูกมอบหมายให้พนักงานได้หลายคน)

	*Business Rules:*
	+ ภูมิภาคหนึ่งสามารถมีร้านค้าหลายร้าน แต่แต่ละร้านจะอยู่ในภูมิภาคเดียวเท่านั้น
	+ ร้านค้าหนึ่งสามารถมีพนักงานหลายคน แต่พนักงานแต่ละคนจะทำงานในร้านค้าเดียวเท่านั้น
	+ พนักงานแต่ละคนจะได้รับมอบหมายตำแหน่งงานเพียงตำแหน่งเดียว แต่ตำแหน่งงานหนึ่งสามารถมีพนักงานได้หลายคน
	+ แต่ละภูมิภาคจะถูกระบุโดยรหัส REGION_CODE ที่ไม่ซ้ำกัน และมีคำอธิบาย
	+ แต่ละร้านค้าจะมีรหัส STORE_CODE ที่ไม่ซ้ำกัน มีชื่อ และบันทึกยอดขายสะสมประจำปี
	+ แต่ละตำแหน่งงานจะถูกระบุโดยรหัส JOB_CODE ที่ไม่ซ้ำกัน และมีคำอธิบายกับค่าจ้างพื้นฐาน
	+ แต่ละพนักงานจะมีรหัส EMP_CODE ที่ไม่ซ้ำกัน และมีข้อมูลประกอบด้วยตำแหน่ง ชื่อ-นามสกุล อักษรย่อ และวันเกิด

	== Create the basic Crow's Foot ERD for DealCo.

	#showybox(image("./er2-2.png"))
]
...
#section[
	...
	*Relationship Types:*
	+ *COURSE 1:N CLASS:* One-to-Many (รายวิชาหนึ่งสามารถมีคลาสเรียนหลายคลาสได้)
	+ *CLASS 1:N ENROLL:* One-to-Many (คลาสเรียนหนึ่งสามารถมีนักเรียนลงทะเบียนได้หลายคน)
	+ *STUDENT 1:N ENROLL:* One-to-Many (นักเรียนหนึ่งคนสามารถลงทะเบียนในหลายคลาสได้)

	*Business Rules:*
	+ รายวิชาหนึ่งสามารถมีคลาสเรียนหลายคลาส แต่แต่ละคลาสจะต้องอิงจากรายวิชาหนึ่งรายวิชาเท่านั้น
	+ คลาสเรียนหนึ่งสามารถมีนักเรียนลงทะเบียนได้หลายคน  แต่แต่ละข้อมูลการลงทะเบียนจะอ้างอิงถึงคลาสเดียวเท่านั้น
	+ นักเรียนหนึ่งคนสามารถลงทะเบียนในหลายคลาส แต่แต่ละข้อมูลการลงทะเบียนจะอ้างอิงถึงนักเรียนเพียงคนเดียว
	+ รายวิชาทุกวิชาจะมีรหัส CRS_CODE ที่ไม่ซ้ำกัน และจะระบุภาควิชา คำอธิบายรายวิชา และจำนวนหน่วยกิต
	+ คลาสเรียนแต่ละคลาสจะมีรหัส CLASS_CODE ที่ไม่ซ้ำกัน และมีข้อมูลเกี่ยวกับ section เวลาเรียน ห้องเรียน และรหัสอาจารย์ผู้สอน
	+ ข้อมูลการลงทะเบียนจะใช้ CLASS_CODE และ STU_NUM เป็นคีย์รวม และจะเก็บเกรดของนักเรียนในคลาสนั้น
	+ นักเรียนแต่ละคนจะมีรหัส STU_NUM ที่ไม่ซ้ำกัน และมีข้อมูลส่วนตัวและข้อมูลทางการศึกษา เช่น ชื่อ วันเกิด GPA และเบอร์โทรศัพท์

	== Create the basic Crow's Foot ERD for Tiny College.

	#showybox(image("./er2-4.png"))
